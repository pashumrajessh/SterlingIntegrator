  ...
  <HTML>
    <HEAD>
      ...
      <!-- button of an element that contains selected sub-elements - contains +/-/nbsp -->
        .bs  {color:red; font-family:'Courier New'; font-weight:bold; text-decoration:none; background-Color:#3300ff}
      <!-- element container -->
      ...
      <!-- markup characters -->
        .m  {color:blue}
      <!-- text node -->
        .tx {font-weight:bold; color:black}
    ...
    <!-- bgcolor="#E3FAFB" -->
    <!-- bgcolor="#D0FDEB" -->
	<BODY class="st"  bgcolor="#E9FEFE" ID="theBody" title="{$scalarExpr}">
		<xsl:apply-templates/>
	</BODY>
...
<xsl:variable name="belongs" select="count($selectedNodes|.)-count($selectedNodes)"/>
  ...
  <xsl:choose>
	  ...
	  <SPAN id="Selected" class="se"><xsl:value-of select="name()"/>
...
<xsl:template match="processing-instruction('xml')">
  ...
  <DIV class="e">
  <SPAN class="b"><xsl:value-of select="string(' ')"/>  </SPAN>
  <SPAN class="m">&lt;?</SPAN>
<xsl:variable name="belongs" select="count($selectedNodes|.)-count($selectedNodes)"/>
  ...
  <xsl:choose>
  	<xsl:when test="$belongs=0">
  		<SPAN id="Selected" class="se">xml <xsl:for-each select="@*"><xsl:value-of select="name()"/>="<xsl:value-of select="."/>" </xsl:for-each></SPAN>
	</xsl:when>
	<xsl:otherwise>
...
<xsl:variable name="belongs" select="count($selectedNodes|.)-count($selectedNodes)"/>
	<xsl:variable name="parentXML" select="normalize-space(vxpathuser:xml(..))"/>
  ...
  <xsl:choose>
  	<xsl:when test="$belongs=0">
		<pre><SPAN id="Selected" class="se">
		<xsl:choose>
			<xsl:when test="$bImplied">
				<xsl:attribute name="title">default/implied</xsl:attribute>
			</xsl:when>
		</xsl:choose>
		<B><font size="+0.5" face="Verdana"><xsl:value-of select="vxpathuser:xml(.)"/></font></B> </SPAN></pre>
	</xsl:when>
	<xsl:otherwise>
		<SPAN>
		<xsl:choose>
			...
			<xsl:otherwise>
				...
				<pre><SPAN class="tx"><font size="+0.1" face="Verdana">
				<xsl:value-of select="." disable-output-escaping="yes"/>
				 </font></SPAN></pre><SPAN class="m">"</SPAN>
			</xsl:otherwise>
		</xsl:choose>
		</SPAN>
	</xsl:otherwise>
...
<xsl:variable name="belongs" select="count($selectedNodes|.)-count($selectedNodes)"/>
  ...
  <xsl:choose>
  	<xsl:when test="$belongs=0">
		<SPAN id="Selected" class="se"> 
		<xsl:value-of select="name()"/>=<xsl:value-of select="."/></SPAN>
	</xsl:when>
	<xsl:otherwise>
		<SPAN class="dt"> <xsl:value-of select="name()"/></SPAN>
		<SPAN class="m">="</SPAN><B class="dt"><xsl:value-of select="."/></B><SPAN class="m">"</SPAN>
	</xsl:otherwise>
  </xsl:choose>
		</xsl:template>
...
<xsl:variable name="belongs" select="count($selectedNodes|.)-count($selectedNodes)"/>
  <xsl:choose>
  	<xsl:when test="$belongs=0">
  		<pre><SPAN id="Selected" class="se"><font size="+0.5" face="Verdana">
		<b><xsl:value-of select="vxpathuser:xml(.)"/></b></font></SPAN></pre>
	</xsl:when>
	<xsl:otherwise>
  		<pre><SPAN class="tx"><font size="+0.5" face="Verdana">
		<b><xsl:value-of select="vxpathuser:xml(.)"/></b></font> 
		</SPAN></pre>
	</xsl:otherwise>
  </xsl:choose>
...
<xsl:variable name="belongs" select="count($selectedNodes|.)-count($selectedNodes)"/>
  ...
  <xsl:choose>
  	<xsl:when test="$belongs=0">
  		<SPAN id="Selected"></SPAN><SPAN id="clean" class="se"><PRE><xsl:value-of select="."/></PRE></SPAN>
	</xsl:when>
	<xsl:otherwise>
  		<SPAN id="clean" class="ci"><PRE><xsl:value-of select="."/></PRE></SPAN>
	</xsl:otherwise>
  </xsl:choose>
  <SPAN class="b"><xsl:value-of select="string(' ')"/></SPAN> <SPAN class="m">--&gt;</SPAN>
...
<xsl:variable name="belongs" select="count($selectedNodes|.)-count($selectedNodes)"/>
  ...
  <xsl:choose>
  	<xsl:when test="$belongs=0">
  		<SPAN id="Selected" class="se"><xsl:value-of select="name()"/></SPAN>
	</xsl:when>
	<xsl:otherwise>
  ...
  <xsl:apply-templates select="@*|@xmlns:*|@xmlns|@xml:*|@dt:*|@d2:*"/><SPAN class="m"> /&gt;</SPAN>
  </DIV></DIV>
</xsl:template>
...
<xsl:variable name="belongs" select="count($selectedNodes|.)-count($selectedNodes)"/>
  ...
  <DIV class="c"><A href="#" onclick="return false" onfocus="h()" class="b">-</A> <SPAN class="m">&lt;</SPAN>
  ...
  <xsl:apply-templates select="@*|@xmlns:*|@xmlns|@xml:*|@dt:*|@d2:*"/> <SPAN class="m">&gt;</SPAN></DIV>
  <DIV><xsl:apply-templates/>
  <DIV><SPAN class="b"><xsl:value-of select="string(' ')"/></SPAN> <SPAN class="m">&lt;/</SPAN>
...
<xsl:template name="getNamespaceNodes" >
  ...
  <xsl:for-each select="namespace::*[position() > 1]">
	<xsl:if test="not(parent::*/parent::*) or not(contains(vxpathuser:xml($parentElementText), vxpathuser:xml(.)))" >
		<xsl:variable name="belongs" select="count($selectedNodes|.)-count($selectedNodes)"/>
		  <SPAN class="m"><xsl:text>&#160;</xsl:text></SPAN>
		  <xsl:choose>
		  	<xsl:when test="$belongs=0">
			<SPAN id="Selected" class="se"> <xsl:value-of select="vxpathuser:xml(.)"/></SPAN>
			</xsl:when>
			<xsl:otherwise>
				<SPAN class="ns"> 
				...
				<SPAN class="m">"</SPAN>
				<!-- <xsl:value-of select="vxpathuser:xml(.)"/></SPAN> -->
			</xsl:otherwise>
		  </xsl:choose>
		<!-- <xsl:text> </xsl:text><xsl:value-of select="vxpathuser:xml(.)"/> -->
	</xsl:if>
...
<DIV class="c" STYLE="margin-left:1em;text-indent:-2em">
  ...
  <A href="#" onclick="return false" onfocus="h()" class="bs">-</A> <SPAN class="m">&lt;</SPAN>
	</xsl:when>
	<xsl:otherwise>
<A href="#" onclick="return false" onfocus="h()" class="b">-</A> <SPAN class="m">&lt;</SPAN>
	</xsl:otherwise>
  </xsl:choose>
  ...
  <SPAN class="m">&gt;</SPAN></DIV>
  <DIV><xsl:apply-templates/>
  <DIV><SPAN class="b"><xsl:value-of select="string(' ')"/></SPAN>
  <SPAN class="m">&lt;/</SPAN>
  <SPAN><xsl:attribute name="class"><xsl:if test="self::xsl:*">x</xsl:if>t</xsl:attribute><xsl:value-of select="name()"/></SPAN><SPAN class="m">&gt;</SPAN></DIV>
  </DIV></DIV>
</xsl:template>
